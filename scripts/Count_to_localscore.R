...
# SET THE WORKING DIRECTORY
  DATADIR <- "C:/Users/file/to/docounts/output" 
  setwd(DATADIR)
...
# two columns: chr (numbered) and chrName (what your chromosomes are called in your angsd output file)
  CHROMFILE <- "C:/Users/file/to/Metadata/chrom_meta_data.txt" 
  chrom_df <- read.table(CHROMFILE, header = TRUE)

# read in doCounts output for each population
  pop1_df <- read.delim(paste0(POP1,"_bayes_R.txt"), header = TRUE, sep = "\t", row.names = NULL)
  pop2_df <- read.delim(paste0(POP2,"_bayes_R.txt"), header = TRUE, sep = "\t", row.names = NULL)
...
###########################################################################
  ...
  write.table(fet_df, file = paste0("C:/Users/file/to/LocalScore/input/",POP2,"_localscore_input.txt"), 
...
write.table(fet_df, file = paste0("C:/Users/directory/input/",POP2,"_postFET.txt"), 
            sep = "\t", row.names = FALSE, col.names = TRUE)
...
#The first line of the table should include the names: chr pos pval
  #fet_df <- fread(paste0("input/",POP2,"_postFET.txt"), head=T) # if rerunning from fet_df output
  setkey(fet_df, chr)
...
#   Then the score function will be $−log10(p−value) − \xi$. 
  ...
  pname= paste0(DATADIR,"/plots/",POP2,"_pvaldist.pdf")
  pdf(pname)
  qplot(pval, data=fet_df, geom='histogram', binwidth=0.1, main='P-values histogram')
  invisible(dev.off())
  
  bname= paste0(DATADIR,"/plots/",POP2,"_blog10.pdf")
  pdf(bname)
...
filename <- paste0(DATADIR,"/output/",POP2,"_tuning_parameter_xi_percentiles.txt")
...
coefsG=coefsGumb(fet_df, Ls=seq(10000,max(seq(10000,80000,10000)[seq(10000,80000,10000)<=min(table(fet_df$chr))]),5000), nSeq=5000)
...
# Determining significance thresholds and writing files
  ...
  sig05name = paste0(DATADIR,"/output/",POP2,"_xi",xi, "_05sig.txt")
  sig01name = paste0(DATADIR,"/output/",POP2,"_xi",xi, "_01sig.txt")
  sig001name = paste0(DATADIR,"/output/",POP2,"_xi",xi, "_001sig.txt")
  sig075name = paste0(DATADIR,"/output/",POP2,"_xi",xi, "_075sig.txt")
...
#Average of significance thresholds --> write to table
  ...
  avname = paste0(DATADIR,"/output/",POP2,"_xi",xi, "_mean_sig.txt")
...
#Chromosome specific thresholds 
  ...
  chrname = paste0(DATADIR,"/output/",POP2,"_xi",xi,"_chr_sig.txt")
  ...
  lsname = paste0(DATADIR,"/output/",POP2,"_xi",xi, ".ls")
...
# Plotting chr-by-chr
  pdf(paste0(DATADIR,"/plots/",POP2,'_ScoreLocalAll_xi',xi,'_nonuniform.pdf'))
...
pdf(paste0(DATADIR,"/plots/",POP2,'_ScoreLocalAll_xi',xi,'.pdf'))
par(mfrow=c(4,1), mar=c(5,2,1,1))
for (g in chrInfo$chr){
