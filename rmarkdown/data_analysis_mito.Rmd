...
x <- as.data.frame(freqs_loc) %>% mutate(hap = seq(1:71))

x1 <- x %>% select(AI,hap) %>% filter(AI != 0)
x2 <- x %>% select(EastBering,hap) %>% filter(EastBering != 0)
x3 <- x %>% select(GOA,hap) %>% filter(GOA != 0)
x4 <- x %>% select(NorthBering,hap) %>% filter(NorthBering != 0)
x5 <- x %>% select(SEAK,hap) %>% filter(SEAK != 0)

op <- par(mfcol = c(3, 2))
ggplot(x1, aes(x="", y=AI, fill=as.character(hap))) +
  geom_bar(stat="identity", width=1, color = "black") +
  coord_polar("y", start=0) +
  labs(title = "Aleutian Islands")
ggplot(x2, aes(x="", y=EastBering, fill=as.character(hap))) +
  geom_bar(stat="identity", width=1, color = "black") +
  coord_polar("y", start=0) +
  labs(title = "East Bering")
ggplot(x3, aes(x="", y=GOA, fill=as.character(hap))) +
  geom_bar(stat="identity", width=1, color = "black") +
  coord_polar("y", start=0) +
  labs(title = "Gulf of Alaska")
ggplot(x4, aes(x="", y=NorthBering, fill=as.character(hap))) +
  geom_bar(stat="identity", width=1, color = "black") +
  coord_polar("y", start=0) +
  labs(title="North Bering")
ggplot(x5, aes(x="", y=SEAK, fill=as.character(hap))) +
  geom_bar(stat="identity", width=1, color = "black") +
  coord_polar("y", start=0) +
