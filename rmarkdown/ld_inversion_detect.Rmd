...
## Make LD matrix for chromosome 54 and chr 100 (most likely inversion)
...
zcat angsd/PCAM-PPLA-wholegenome_polymorphic_CM023352.1.beagle.gz | cut -f 1 | sed 's/_/\t/1' | gzip > angsd/global_snp_list_PCAM-PPLA-wholegenome_polymorphic_CM023352.1.txt.gz
```
...
mafs <- read_tsv("/fs/cbsubscb16/storage/rkc/angsd/PCAM-PPLA_CM023352.1_global.mafs.gz")
...
## Using full outlier region from chr 100 so just directly filter global snp list
# zcat angsd/global_snp_list_PCAM-PPLA-wholegenome_polymorphic_CM023352.1.txt.gz | sed -n -e '1p' -e '14635,15268p;15269q' | gzip > angsd/global_snp_list_PCAM-PPLA-wholegenome_polymorphic_CM023352.1_outlierPlus.txt.gz
```

#### select sites from beagle file and remove chr and pos info
```{bash eval = F, include = T}
...
## Chr 100
zcat angsd/PCAM-PPLA-wholegenome_polymorphic_CM023352.1_outlierPlus.beagle.gz | tail -n +2 | cut -f 4- | gzip > angsd/PCAM-PPLA-wholegenome_polymorphic_CM023352.1_outlierPlus_noheader.beagle.gz
```
...
nohup bash /fs/cbsubscb16/storage/genomic-data-analysis/scripts/get_ld.sh /fs/cbsubscb16/storage/rkc/angsd/ PCAM-PPLA-wholegenome_polymorphic_CM023306.1_outlierPlus_noheader.beagle.gz global_snp_list_PCAM-PPLA-wholegenome_polymorphic_CM023306.1.txt.gz 125 16 '/programs/ngsLD-1.1.1/ngsld.sif /ngsLD-1.1.1/ngsLD' > /fs/cbsubscb16/storage/rkc/nohups/run_ngsLD_bam_list_PCAM-PPLA-wholegenome_polymorphic_CM023306.1_125.nohup &

## Chr 100
nohup bash /fs/cbsubscb16/storage/genomic-data-analysis/scripts/get_ld.sh /fs/cbsubscb16/storage/rkc/angsd/ PCAM-PPLA-wholegenome_polymorphic_CM023352.1_outlierPlus_noheader.beagle.gz global_snp_list_PCAM-PPLA-wholegenome_polymorphic_CM023352.1_outlierPlus_nh.txt.gz 100 16 '/programs/ngsLD-1.1.1/ngsld.sif /ngsLD-1.1.1/ngsLD' > /fs/cbsubscb16/storage/rkc/nohups/run_ngsLD_bam_list_PCAM-PPLA-wholegenome_polymorphic_CM023352.1_100.nohup &
...
#### Plot Chr 100 LD 
```{bash eval=F, include=T}
cat /fs/cbsubscb16/storage/rkc/angsd/PCAM-PPLA-wholegenome_polymorphic_CM023352.ld | bash /fs/cbsubscb16/storage/rkc/scripts/ld_blocks.sh \
CM023352.1 \
10600773 \
11192083
```
