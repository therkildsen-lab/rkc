...
nohup bash /fs/cbsubscb16/storage/genomic-data-analysis/scripts/get_ld.sh /fs/cbsubscb16/storage/rkc/angsd/ PCAM-PPLA-wholegenome_polymorphic_CM023306.1_downsampledEM_noheader.beagle.gz global_snp_list_PCAM-PPLA-wholegenome_polymorphic_CM023306.1_downsampledEM.txt.gz 12450 16 '/programs/ngsLD-1.1.1/ngsld.sif /ngsLD-1.1.1/ngsLD' > /fs/cbsubscb16/storage/rkc/nohups/run_ngsLD_bam_list_PCAM-PPLA-wholegenome_polymorphic_CM023306.1_downsampledEM_400.nohup &
```

## Plot LD heatmap to see if there is an obvious block of ld
Just plotting the start becaus there is too much data. Should downsample later
```{bash eval=F, include=T}
...
cat /fs/cbsubscb16/storage/rkc/angsd/PCAM-PPLA-wholegenome_polymorphic_CM023306.ld | awk -v chr=CM023306.1 -v min=19500000 -v max=32000000 'BEGIN{print "snp1\tsnp2\tdist\tr2p\tD\tDp\tr2"} {split($1,pos1,":"); split($2,pos2,":")} pos1[1]==chr && pos1[2]>=min && pos1[2]<=max && pos2[1]==chr && pos2[2]>=min && pos2[2]<=max' | cut -f 1-7 > PCAM-PPLA_CM023306.1_downsampledEM_12450_premat.ld
```

```{r}
r <- read.table("/fs/cbsubscb16/storage/rkc/angsd/PCAM-PPLA_CM023306.1_downsampledEM_12450_premat.ld", header=TRUE, stringsAsFactors=FALSE)
...
extractClusters(ldna, m, min.edges = 18, plot.tree = T, extract = F)
```
...
```{r}
par(mfcol=c(1,2))
plotLDnetwork_csj(LDmat=m, option = 2, threshold = 0.37)
plotLDnetwork_csj(LDmat=m, option = 2, threshold = 0.34)
```
