...
```{r filter sample table and beagle file for even sample sizes}
...
```{r filter sample table and beagle file for Bristol Bay}
sample_table <- read_tsv("/fs/cbsubscb16/storage/rkc/sample_lists/sample_table.tsv")
...
```{bash cut EastBering columns, eval = F, include = T}
...
PCA_continuous_var(genome_cov_EastBering, sample_table_eastbering_prib$ABLG, sample_table_eastbering_prib$year, 1, 2, "EastBering_label_pca", show.ellipse = F, show.line = F, show.label = F)
EastBering_label_pca_plot <- ggplot(EastBering_label_pca, aes(x=PC1,y=PC2)) +
  geom_point(aes(color = population)) +
  #geom_text(aes(label = individual)) +
  cowplot::theme_cowplot() +
    xlab(paste0("PC1 ", round(EastBering_label_pca_x_var,2),"%")) +
    ylab(paste0("PC2 ",round(EastBering_label_pca_y_var,2),"%"))
EastBering_label_pca_plot
```
