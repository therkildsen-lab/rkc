...
sample_table_pribs <- metadata %>% 
  ...
  mutate(Loc = ifelse(str_detect(population, "Pri") | str_detect(population, "Sai"), "Pribilof_Is", Locality)) %>% 
...
#ggsave("/fs/cbsubscb16/storage/rkc/figures/All_pop_pca.png", device = "png")
```
...
read_csv("/fs/cbsubscb16/storage/rkc/sample_lists/sample_table.tsv")
...
```{r include=F}
# copy this into an R script and run for now. Will have to figure out how to plot in poloting window so that I can save as a pdf
