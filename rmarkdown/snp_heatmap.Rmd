...
GOA_majors <- GOA_beage %>% select(seq(4,ncol(GOA_beage), by = 3))
...
#### Dosage boxplot
```{r}
dosage_cand_long %>% group_by(Loc, ABLG) %>% summarize(med = median(allele_dosage, na.rm = T))

dosage_cand_long %>% group_by(Loc, ABLG) %>%  
